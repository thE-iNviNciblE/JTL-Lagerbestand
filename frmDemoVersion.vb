Public Class frmDemoVersion
    ...
    Private Sub frmDemoVersion_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        ...
        Dim strServerInfo() As String = getHTTPResponseMessage("https://api.bludau-media.de/SafeSandy/fetchAuth.php?key=" & System.Uri.EscapeDataString(gbl_KeyCode), mgetUpdaterMessage.getAuthCode, True)

        If strServerInfo(0) = "ADD_AUTHCODE" Then

            Dim strServerInfo2() As String = getHTTPResponseMessage("https://api.bludau-media.de/SafeSandy/SendAuthCode.php?key=" & System.Uri.EscapeDataString(gbl_KeyCode) & "&programID=12&programname=JTL Lagerbestand Report&demoende=" & Date.Now.AddDays(15), mgetUpdaterMessage.getAuthCode, True)
            'MessageBox.Show(strServerInfo2(0))
            strServerInfo = getHTTPResponseMessage("https://api.bludau-media.de/SafeSandy/fetchAuth.php?key=" & System.Uri.EscapeDataString(gbl_KeyCode), mgetUpdaterMessage.getAuthCode, True)
        End If
